...
use cw_common::xcall_msg::XCallMsg as XCallExecuteMsg;
...
impl TestContext {
    ...
    pub fn get_assetmanager_app(&self) -> Addr {
        self.contracts.get(&TestApps::AssetManager).unwrap().clone()
    }

    pub fn get_cw20token_app(&self) -> Addr {
        self.contracts.get(&TestApps::CW20Token).unwrap().clone()
    }
}
